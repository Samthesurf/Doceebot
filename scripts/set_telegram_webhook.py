...
def main() -> int:
    ...
    app_base_url = require_env("APP_BASE_URL").rstrip("/")
    webhook_url = f"{app_base_url}/webhooks/telegram/webhook"
    ...
    telegram_api_post(token, "setWebhook", payload)
    print(f"Telegram webhook set to: {webhook_url}")
    if "secret_token" in payload:
        print("Telegram secret-token header is enabled.")
    else:
        print("Warning: Telegram secret-token header is disabled for this local setup.")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"set_telegram_webhook failed: {exc}", file=sys.stderr)
